...
#### This script is only used for RCM-triCNN trial with all windows for the inputs
...
class BS_LS_DataSet_Prep():
    ...
    def get_train_test_keys(self):
        ...
        ## first 8000 used for best base and rcm model selection
    ...
    def seq_to_tensor(self, data_keys, rcm_folder, is_rcm=None, is_upper_lower_concat=None):
        ...
        for key in data_keys:
            ...
            if is_rcm:
                ...
                # rcm_values_concate = torch.from_numpy(np.concatenate([flanking_rcm_kmers,\
                #                                                 upper_rcm_kmers,\
                #                                                 lower_rcm_kmers], axis=1)).to(torch.float32)
                # rcm_values_scores.append(rcm_values_concate)
        ...
        if is_rcm:
            # print(flanking_rcm_dict_list)
            # print(upper_rcm_dict_list)
            # print(lower_rcm_dict_list)
            # all_torch_rcm_features = torch.stack(rcm_values_scores, dim=0)
        ...
        if is_upper_lower_concat and is_rcm:
            ...
            # return all_torch_feature, all_torch_rcm_features, all_torch_label
        ...
        if not is_upper_lower_concat and is_rcm:
            ...
            # return all_torch_upper_feature, all_torch_lower_feature, all_torch_rcm_features, all_torch_label
...
# class BS_LS_upper_lower_rcm(Dataset):
#     def __init__(self, include_rcm, seq_upper_feature, seq_lower_feature, rcm, label):
#         # construction of the map-style datasets
#         # data loading
#         self.include_rcm = include_rcm
#
